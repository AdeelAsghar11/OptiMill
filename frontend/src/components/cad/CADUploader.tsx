      ...
      <AnimatePresence mode="wait">
        {!result ? (
          <motion.div 
            ...
            className="glass-card p-6 md:p-16 border-2 border-dashed border-white/10 flex flex-col items-center justify-center space-y-8 text-center group cursor-pointer hover:border-blue-500/50 hover:bg-white/5 transition-all"
          >
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto px-4">
              <label
                htmlFor="cad-upload"
                className="w-full sm:w-auto px-8 py-4 bg-white/5 hover:bg-white/10 rounded-2xl font-bold transition-all border border-white/10 text-xs md:text-sm uppercase tracking-widest text-center"
              >
              ...
              {file && !loading && (
                <button
                  ...
                  className="btn-premium w-full sm:w-auto px-12 py-4 rounded-2xl text-white font-black uppercase tracking-widest text-xs md:text-sm shadow-2xl"
                >
                  Begin Analysis
                </button>
              )}
          ...
          >
            ...
            <div className="lg:col-span-5 space-y-6 md:space-y-8 order-2 lg:order-1">
              <div className="glass-card p-3 md:p-4 rounded-3xl md:rounded-[2.5rem] overflow-hidden sticky top-24 md:top-32">
                <div className="flex items-center justify-between mb-4 px-3 md:px-4">
                  <h4 className="text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-500 flex items-center gap-2">
                ...
                <div className="mt-4 p-4 md:p-6 bg-blue-600/5 border-t border-white/5 rounded-b-2xl md:rounded-b-[2rem]">
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-2 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-blue-400">
                    <span>Geometry Status: Verified</span>
                    <span className="hidden sm:inline">Ready for fabrication</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-7 space-y-8 md:space-y-12 pb-24 order-1 lg:order-2">
              <header className="space-y-3 md:space-y-4">
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                  ...
                  <span className="text-slate-500 text-[10px] md:text-xs font-medium">{new Date().toLocaleDateString()} — ID: {result.id.slice(0,8)}</span>
                </div>
                <h2 className="text-3xl md:text-5xl font-black tracking-tight">{result.file_name}</h2>
              </header>

              {/* Top Level Stats */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                <div className="glass p-6 md:p-8 rounded-3xl md:rounded-[2rem] border border-white/5 relative overflow-hidden group">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl md:text-5xl font-black text-blue-400">{result.feasibility_score}</span>
                    <span className="text-base md:text-lg font-bold text-slate-600">/ 100</span>
                  </div>
                </div>
                <div className="glass p-6 md:p-8 rounded-3xl md:rounded-[2rem] border border-white/5 relative overflow-hidden group">
                   <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  ...
                  <div className="text-xl md:text-2xl font-black capitalize text-white mb-1">{result.analysis.design_type || "Mechanical Part"}</div>
              ...
              <div className="glass-card p-6 md:p-10 space-y-8 md:space-y-10">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-base md:text-lg font-bold capitalize">{result.analysis.complexity}</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-base md:text-lg font-bold">{result.process_recommendation}</span>
                    </div>
                  </div>
                </div>

                <div className="p-6 md:p-8 rounded-3xl bg-gradient-to-br from-blue-600/10 to-indigo-600/10 border border-blue-500/20">
                  ...
                  <p className="text-[10px] text-slate-500 mt-4 font-medium uppercase tracking-widest leading-relaxed">
                ...
                <div className="space-y-8 pt-4">
                  ...
                  <ShopRecommendationList cadFileId={result.id} directShopId={directShopId || undefined} />
                </div>
